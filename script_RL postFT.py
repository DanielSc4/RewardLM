...
def main(config_name: str,):
    ...
    if load_from_peft:
        lora_config = LoraConfig.from_pretrained(config['model_id'])
        original_pretrained_model_id = lora_config.base_model_name_or_path
        if debug:
            print(f'Obtaining original model: {original_pretrained_model_id}')
    else:
        lora_config = LoraConfig(
            **config['LoRA_config'],
        )
        original_pretrained_model_id = config['model_id']
    ...
    if load_from_peft:
        tokenizer = AutoTokenizer.from_pretrained(original_pretrained_model_id)     # downloads the original pretrained tokenizer
    else:
